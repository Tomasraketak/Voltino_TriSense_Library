...
bool TriSense::beginAll(TriSenseMode mode, uint8_t spiCsPin, uint32_t spiFreq) {
  ...
  if (_mode == MODE_I2C) {
    // Pro I2C ignorujeme CS pin i frekvenci, použijeme defaults (Wire 400kHz)
    if (!imu.begin(BUS_I2C)) return false;
    imu.setODR(ODR_1KHZ); 
  } else {
    // Pro SPI/Hybrid použijeme zadané (nebo defaultní) parametry
...
// ... Zbytek souboru (TriSenseFusion, SimpleTriFusion, AdvancedTriFusion) je identický jako v předchozí verzi ...
// ... Vlož sem implementaci Fusion tříd z mé předchozí odpovědi ...
...
bool SimpleTriFusion::update() {
  float ax, ay, az, gx, gy, gz; if (!_imu->readFIFO(ax, ay, az, gx, gy, gz)) return false;
  ax -= accelOffset[0]; ay -= accelOffset[1]; az -= accelOffset[2]; gx -= gyroOffset[0];  gy -= gyroOffset[1];  gz -= gyroOffset[2];
  unsigned long now = micros(); float dt = (now - lastTime) / 1000000.0f; if (dt <= 0) dt = 0.000125f; lastTime = now;
  gyroIntegration(gx * PI/180.0f, gy * PI/180.0f, gz * PI/180.0f, dt);
  lastAx = ax; lastAy = ay; lastAz = az; lastGx = gx; lastGy = gy; lastGz = gz; return true;
}
...
void AdvancedTriFusion::complementaryCorrection(float ax, float ay, float az, float mx, float my, float mz, float correction_dt) {
  float totalAccelG = sqrt(ax * ax + ay * ay + az * az); float recipNorm = 1.0f / totalAccelG; ax *= recipNorm; ay *= recipNorm; az *= recipNorm;
  float magStrength = sqrt(mx * mx + my * my + mz * mz);
  float vx = 2.0f * (q[1] * q[3] - q[0] * q[2]); float vy = 2.0f * (q[0] * q[1] + q[2] * q[3]); float vz = q[0] * q[0] - q[1] * q[1] - q[2] * q[2] + q[3] * q[3];
  float ex = (ay * vz - az * vy); float ey = (az * vx - ax * vz);
  float g_gain_accel_raw = gaussianGain(totalAccelG, accRef, accSigma); float g_gain_mag_raw = gaussianGain(magStrength, magRef, magSigma);
  float final_accel_gain = g_gain_accel_raw * maxAccelGain; float final_mag_gain = g_gain_mag_raw * maxMagGain;
  float roll_rad, pitch_rad, yaw_rad; quaternionToEuler(roll_rad, pitch_rad, yaw_rad);
  float tilt_gain_roll = gaussianGain(fabs(roll_rad * 180.0f/PI), 0.0f, magTiltSigmaDeg); float tilt_gain_pitch = gaussianGain(fabs(pitch_rad * 180.0f/PI), 0.0f, magTiltSigmaDeg);
  final_mag_gain *= tilt_gain_roll * tilt_gain_pitch;
  float roll_corr, pitch_corr, yaw_corr; getCorrectionAngles(ax * totalAccelG, ay * totalAccelG, az * totalAccelG, mx, my, mz, roll_corr, pitch_corr, yaw_corr);
  float yaw_deg = yaw_rad * 180.0f / PI; if (yaw_deg < 0) yaw_deg += 360.0f; if (yaw_deg >= 360.0f) yaw_deg -= 360.0f;
  float delta_yaw_deg = yaw_corr - yaw_deg; if (delta_yaw_deg > 180.0f) delta_yaw_deg -= 360.0f; if (delta_yaw_deg < -180.0f) delta_yaw_deg += 360.0f;
  float delta_yaw_rad = delta_yaw_deg * PI / 180.0f;
  if (lastDeltaYawRad * delta_yaw_rad < 0.0f) gyroBiasZ = 0.0f; lastDeltaYawRad = delta_yaw_rad;
  gyroBiasZ -= yawKi * delta_yaw_rad * final_mag_gain * correction_dt;
  float w_x = final_accel_gain * ex * correction_dt; float w_y = final_accel_gain * ey * correction_dt; float w_z = final_mag_gain * delta_yaw_rad * correction_dt;
  float q0_old = q[0], q1_old = q[1], q2_old = q[2], q3_old = q[3];
  q[0] += 0.5f * (-q1_old * w_x - q2_old * w_y - q3_old * w_z); q[1] += 0.5f * (q0_old * w_x + q2_old * w_z - q3_old * w_y);
  q[2] += 0.5f * (q0_old * w_y - q1_old * w_z + q3_old * w_x); q[3] += 0.5f * (q0_old * w_z + q1_old * w_y - q2_old * w_x);
  recipNorm = 1.0f / sqrt(q[0] * q[0] + q[1] * q[1] + q[2] * q[2] + q[3] * q[3]); q[0] *= recipNorm; q[1] *= recipNorm; q[2] *= recipNorm; q[3] *= recipNorm;
}
bool AdvancedTriFusion::update() {
  float ax, ay, az, gx, gy, gz; if (!_imu->readFIFO(ax, ay, az, gx, gy, gz)) return false;
  lastAx = ax - accelOffset[0]; lastAy = ay - accelOffset[1]; lastAz = az - accelOffset[2];
  lastGx = gx - gyroOffset[0]; lastGy = gy - gyroOffset[1]; lastGz = gz - gyroOffset[2];
  unsigned long now = micros(); float dt = (now - lastTime) / 1000000.0f; if (dt <= 0) dt = 0.0000625f; lastTime = now;
  gyroIntegration(lastGx * PI/180.0f, lastGy * PI/180.0f, (lastGz - gyroBiasZ) * PI/180.0f, dt);
